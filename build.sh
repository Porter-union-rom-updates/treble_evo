echo "  thanks for build infinity x"

ROOT_DIR="$(pwd)"
cd $ROOT_DIR

EVO_VERSION="$(awk '/EVO_VERSION := / {print $3}' $ROOT_DIR/vendor/infinity/config/version.mk)"
ANDROID_BUILD_VERSION="ap4a"

echo "Building infinity X version $EVO_VERSION ($ANDROID_BUILD_VERSION)"
echo "---------------------------"
...
compress() {
    ...
    mv system.img.xz $HOME/Downloads/infinity_arm64_$variant-$EVO_VERSION-unofficial-$RELEASE_DATE.img.xz
}

build() {
    cd $ROOT_DIR
    lunch infinity_arm64_$variant-$ANDROID_BUILD_VERSION-userdebug
...
echo "----- OTA -----"
echo "{
    \"version\": \"$RELEASE_DATE_FMT (infinity $EVO_VERSION)\",
    \"date\": \"$START_TIME\",
    \"variants\": [
        {
            \"name\": \"infinity_arm64_bgN\",
            \"size\": \"$normal_size\",
